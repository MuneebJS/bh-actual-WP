...
function enside_setup() {
	...
	// Activate theme
	update_option('enside_license_key_status', 'activated');
...
function enside_scripts() {
	...
	wp_enqueue_style('enside-products-section', get_template_directory_uri() . '/css/products-section.css', array(), '1.0', 'all');
	wp_enqueue_style('enside-office-address', get_template_directory_uri() . '/css/office-address.css', array(), '1.0', 'all');
	...
	wp_enqueue_script('enside-script', get_template_directory_uri() . '/js/template.js', array('jquery', 'bootstrap', 'easing', 'enside-select2', 'owl-carousel', 'nanoscroller', 'mixitup', 'tweenmax'), '1.2', true);
...
/**
 * Resolve team avatar: prefer Bitmoji/exported PNG or WebP in img/team/{slug}.*, else bundled SVG.
 */
if ( ! function_exists( 'enside_team_member_avatar_url' ) ) :
	function enside_team_member_avatar_url( $slug ) {
		$slug = sanitize_file_name( $slug );
		if ( '' === $slug ) {
			return '';
		}
		$dir = get_template_directory() . '/img/team/';
		$uri = get_template_directory_uri() . '/img/team/';
...
/**
 * Output core products and team sections on the homepage (after hero slider when detected).
 */
if ( ! function_exists( 'enside_insert_core_products_after_hero' ) ) :
	function enside_insert_core_products_after_hero( $content ) {
		...
		foreach ( $markers as $marker ) {
			...
			if ( false !== $pos ) {
				...
				return substr( $content, 0, $insert_at ) . $inject_html . substr( $content, $insert_at );
			}
		}

		// Slider markup not recognized: append so order stays below hero-like content when possible.
		return $content . $inject_html;
	}
endif;
add_filter( 'the_content', 'enside_insert_core_products_after_hero', 12 );
...
function enside_admin_scripts() {
	...
	wp_register_script('enside-js-wp', get_template_directory_uri() . '/js/js-wp.min.js', array(), '1.0', true);
	wp_enqueue_script('enside-js-wp');
...
*/
if (!function_exists('enside_wp_kses_data')) :
function enside_wp_kses_data($data) {
 ...
 */
function enside_theme_support() {
		remove_theme_support('widgets-block-editor');
}
add_action('after_setup_theme', 'enside_theme_support');
